...
import type { Category, Reference, ColorPaletteData } from '../types';
...
interface CategorySectionProps {
  ...
  onColorPaletteChange?: (palette: ColorPaletteData) => void;
}
    ...
    <div className="category-card">
      ...
      >
        <div className="flex items-center gap-3">
          <div className={`w-2 h-2 rounded-full flex-shrink-0 ${hasRefs || hasColors ? 'bg-green-500' : 'bg-gray-300'}`} />
          <div>
      ...
      {isExpanded && (
        <div className="px-4 pb-4 border-t border-gray-100">
          ...
          {/* Color palette editor (colors category only) */}
          {isColorCategory && colorPalette && onColorPaletteChange && (
            <ColorPaletteEditor
              palette={colorPalette}
              hasReferences={hasRefs}
              isExtracting={isExtractingColors ?? false}
              onExtract={onExtractColors ?? (() => {})}
              onChange={onColorPaletteChange}
            />
          )}

          {/* Add buttons (shown for all categories, including color) */}
          <div
          ...
          {!hasRefs && !isColorCategory && (
            <p className="mt-3 text-xs text-gray-400 text-center py-2">
              URLまたは画像を追加して参考資料を登録してください
            </p>
          )}
          {!hasRefs && isColorCategory && !hasColors && (
            <p className="mt-3 text-xs text-gray-400 text-center py-2">
              参考資料を追加してAIで色を抽出するか、手動でカラーを登録してください
            </p>
          )}
        </div>
      )}
